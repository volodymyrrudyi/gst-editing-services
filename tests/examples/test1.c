...
main (int argc, gchar ** argv)
{
  ...
  /* We create a simple material able to extract GESTimelineTestSource */
  ges_material_new_simple (&src_material, GES_TYPE_TIMELINE_TEST_SOURCE, NULL);

  /* Add sources to our layer */
  ges_timeline_layer_add_material (layer, src_material, 0, 0, GST_SECOND, 1,
      GES_TRACK_TYPE_UNKNOWN);
  source = ges_timeline_layer_add_material (layer, src_material, GST_SECOND, 0,
      GST_SECOND, 1, GES_TRACK_TYPE_UNKNOWN);
  g_object_set (source, "freq", 480.0, "vpattern", 2, NULL);
  ges_timeline_layer_add_material (layer, src_material, 2 * GST_SECOND, 0,
      GST_SECOND, 1, GES_TRACK_TYPE_UNKNOWN);
